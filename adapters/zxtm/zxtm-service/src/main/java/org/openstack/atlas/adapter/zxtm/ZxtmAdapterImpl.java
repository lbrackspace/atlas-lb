...
public class ZxtmAdapterImpl implements ReverseProxyLoadBalancerAdapter {
    ...
    public void createLoadBalancer(LoadBalancerEndpointConfiguration config, LoadBalancer lb)
        ...
        try {
            ...
            throw new AdapterRollBackException(rollBackMessage, e);
        }
    ...
    public void updateProtocol(LoadBalancerEndpointConfiguration config, Integer lbId, Integer accountId, LoadBalancerProtocol protocol)
        ...
        try {
            ...
            throw new AdapterRollBackException("Update protocol request canceled.", e);
        }
    ...
    public void updatePort(LoadBalancerEndpointConfiguration config, Integer lbId, Integer accountId, Integer port)
        ...
        try {
            ...
            throw new AdapterRollBackException("Update port request canceled.", e);
        }
    }
    ...
    public void setLoadBalancingAlgorithm(LoadBalancerEndpointConfiguration config, Integer lbId, Integer accountId, LoadBalancerAlgorithm algorithm)
        ...
        try {
            ...
            throw new AdapterRollBackException("Update algorithm request canceled.", e);
        }
    }
    ...
    public void addVirtualIps(LoadBalancerEndpointConfiguration config, LoadBalancer lb)
        ...
        for (LoadBalancerJoinVip6 loadBalancerJoinVip6ToAdd : lb.getLoadBalancerJoinVip6Set()) {
            ...
            try {
                ...
                throw new AdapterRollBackException("Cannot derive name from IPv6 virtual ip.", e);
            }
        }
    ...
    public void deleteVirtualIps(LoadBalancerEndpointConfiguration config, LoadBalancer lb, List<Integer> vipIds)
        ...
        if (!trafficIpGroupNamesToDelete.isEmpty()) {
            try {
                ...
                throw new AdapterRollBackException(rollBackMessage, re);
            }
        }
    }
    ...
    public void setRateLimit(LoadBalancerEndpointConfiguration config, int id, int accountId, RateLimit rateLimit) throws RemoteException, InsufficientRequestException, AdapterRollBackException {
        ...
        try {
            ...
            throw new AdapterRollBackException("Add rate limit request canceled.", e);
        }
    ...
    public void updateRateLimit(LoadBalancerEndpointConfiguration config, int id, int accountId, RateLimit rateLimit) throws RemoteException, InsufficientRequestException, AdapterRollBackException {
        ...
        try {
            ...
            throw new AdapterRollBackException("Update rate limit request canceled.", e);
        }
    }
    ...
    public void changeHostForLoadBalancer(LoadBalancerEndpointConfiguration config, LoadBalancer lb, Host newHost) throws RemoteException, InsufficientRequestException, AdapterRollBackException {
        final String trafficManagerName = newHost == null ? config.getHostName() : newHost.getHostName();
        ZxtmServiceStubs serviceStubs = getServiceStubs(config);

        List<String> tManagersList = config.getFailoverHostNames();
        tManagersList.add(trafficManagerName);
        String[] allTrafficManagers = tManagersList.toArray(new String[config.getFailoverHostNames().size()]);
    ...
    public void deleteConnectionThrottle(LoadBalancerEndpointConfiguration config, Integer lbId, Integer accountId)
        ...
        } catch (AdapterRollBackException zre) {
            throw new AdapterRollBackException(rollBackMessage, zre);
        }

        LOG.info(String.format("Successfully deleted connection throttle for node pool '%s'.", poolName));
    }

    private void zeroOutConnectionThrottleConfig(LoadBalancerEndpointConfiguration config, Integer loadBalancerId, Integer accountId) throws RemoteException, InsufficientRequestException, AdapterRollBackException {
    ...
    private void createNodePool(LoadBalancerEndpointConfiguration config, Integer loadBalancerId, Integer accountId, Collection<Node> allNodes) throws RemoteException, InsufficientRequestException, AdapterRollBackException {
        ZxtmServiceStubs serviceStubs = getServiceStubs(config);
