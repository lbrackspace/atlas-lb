...
public class VTMRestClientTest {
    ...
    public static class whenUpdatingAnItem {
        ...
        @Test
        public void restoreBackupShouldNotReturnNull() throws Exception{
            String tm = "z2.rackspace.com";
            when(requestManager.updateItem(Matchers.<URI>any(), Matchers.<Client>any(), Matchers.<String>any(), Matchers.<Bandwidth>any(), Matchers.<MediaType>any())).thenReturn(mockedResponse);
            Backup backup2 = vtimRestClient.restoreBackup(vsName, backup, tm);
            verify(requestManager, times(1)).updateItem(Matchers.<URI>any(), Matchers.<Client>any(), Matchers.<String>any(), Matchers.<Bandwidth>any(), Matchers.<MediaType>any());
            Assert.assertNotNull(vsName, backup2);
        }

        @Test(expected = VTMRestClientException.class)
        public void restoreBackupShouldThrowException() throws VTMRestClientException, VTMRestClientObjectNotFoundException, URISyntaxException {
            String tm = "z2.rackspace.com";
            backup.setProperties(null);
            Backup backup1 = vtimRestClient.restoreBackup(vsName, backup, tm);
            verify(requestManager, times(1)).updateItem(Matchers.<URI>any(), Matchers.<Client>any(), Matchers.<String>any(), Matchers.<Bandwidth>any(), Matchers.<MediaType>any());
            Assert.assertNull(backup1);
        }
