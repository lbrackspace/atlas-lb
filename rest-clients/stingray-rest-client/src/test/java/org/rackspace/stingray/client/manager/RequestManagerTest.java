...
//            builder = mock(ClientBuilder.class);
//            //requestManager = mock(RequestManagerImpl.class);
//
//            when(client.target(anyString())).thenReturn(webResource);
////            when(webResource.request(Matchers.<MediaType>any())).thenReturn(builder);
//            when(client.target(anyString()).request().get(Response.class)).thenReturn(mockedResponse);
//
//        }
//
...
//            setupMocks();
//
//            Response response = requestManager.getItem(getPoolPath(), client, vsName, MediaType.APPLICATION_JSON_TYPE);
//
