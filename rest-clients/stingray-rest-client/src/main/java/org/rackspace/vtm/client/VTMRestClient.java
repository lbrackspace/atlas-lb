...
public class VTMRestClient extends VTMRestClientManager {
    ...
    public Backup restoreBackup(String name, Backup backup, String trafficManagerName) throws VTMRestClientException, VTMRestClientObjectNotFoundException, URISyntaxException{
        String statsEndpoint = endpoint.toString().split("config")[0];
        endpoint = new URI(statsEndpoint);
        name = name + "?restore";
        String path = "status/" + trafficManagerName + "/"+ ClientConstants.STATUS_BACKUPS + "/";
