...
public class CaFrame extends javax.swing.JFrame {
    ...
    private void printKnownPemObject(Object obj) {
        ...
        if (obj instanceof KeyPair) {
            ...
            try {
                logDbg("%s", RSAKeyUtils.objToString(kp.getPublic()));
            ...
            try {
                ...
                pth.write("Could not decode CSR");
            }
            logDbg("%s\n", msg);
            ...
            String certStr;
            try {
                certStr = CertUtils.certToStr(CertUtils.getX509CertificateObject(cert));
                logDbg("%s", certStr);
            } catch (NotAnX509CertificateException ex) {
                pth.write("Unable to decode x509 certificate.");
            }
        }
    ...
    private void GenerateCsrButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_GenerateCsrButtonActionPerformed
        ...
        switch (rsaMapper.getSelectedId()) {
            ...
            default:
                ...
                try {
                    ...
                    keyBytes = PemUtils.toPemBytes(kp);
        ...
        try {
            csrBytes = PemUtils.toPemBytes(csr);
        } catch (PemException ex) {
    ...
    private void signCSRButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_signCSRButtonActionPerformed
        ...
        try {
            req = (PKCS10CertificationRequest) PemUtils.fromPemBytes(csrBytes);
        } catch (RsaException ex) {
        ...
        if (selfSignCA.isSelected()) {
            ...
            try {
                crtBytes = PemUtils.toPemBytes(crt);
            } catch (PemException ex) {
            ...
            try {
                caCrt = (X509CertificateHolder) PemUtils.fromPemBytes(caCrtBytes);
            } catch (RsaException ex) {
    ...
    private void verifyKeyAndCertButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_verifyKeyAndCertButtonActionPerformed
        ...
        try {
            ...
            key = (KeyPair) PemUtils.fromPemBytes(keyPem);
        } catch (IOException | ClassCastException | PemException ex) {
            logError("Error reading key from \"%s\"\n%s\n", keyFile, getEST(ex));
            return;
        }
        try {
            certPem = RsaFileUtils.readFileToByteArray(certFile);
            x509Holder = (X509CertificateHolder) PemUtils.fromPemBytes(certPem);
        } catch (IOException | ClassCastException | PemException ex) {
            logError("Error reading cert from \"%s\"\n%s\n", certFile, getEST(ex));
            return;
        }

        errors = RSAKeyUtils.verifyKeyAndCert(key, x509Holder);
        if (errors.size() > 0) {
    ...
    private void verifyKeyCertChainActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_verifyKeyCertChainActionPerformed
        ...
        for (X509CertificateHolder root : rootCAs) {
            roots.add((X509CertificateHolder) root);
        }
    ...
    private void loadChainerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loadChainerButtonActionPerformed
        for (X509MapValue mapVal : x509MapValList) {
            X509CertificateHolder x509obj = mapVal.getX509CertificateHolder();
    ...
    private X509CertificateHolder readCrtPathFN() {
        File file = new File(crtPathFN.getText());
        byte[] pemBytes;
        X509CertificateHolder x509obj;
    ...
    private void displayChain(List<X509CertificateHolder> certs) {
        for (int i = 0; i < certs.size(); i++) {
            String pemString;
            X509CertificateHolder x509Holder;
            X509Inspector x509reader;
            X509CertificateHolder curX509 = certs.get(i);
            x509reader = X509Inspector.newX509Inspector(curX509);
            x509Holder = x509reader.getX509CertificateHolder();
            try {
                pemString = PemUtils.toPemString(x509Holder);
            } catch (PemException ex) {
            ...
            try {
                ...
                pth.write("PubkeyMod: UNKNOWN\n");
            }
            pth.write("%s\n", pemString);
            Set<X509MapValue> resultSet = x509Map.getX509CertificateHolder(x509Holder);
    ...
    private void buildNieveChainButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buildNieveChainButtonActionPerformed
        X509CertificateHolder x509Holder = readCrtPathFN();
        if (x509Holder == null) {
            return; // The exception was already reported
        }
        X509CertificateHolder x509;
        x509 = (X509CertificateHolder) x509Holder;
        List<X509CertificateHolder> certs = x509Chainer.buildPath(x509);
    ...
    private void clearRootCAsButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_clearRootCAsButtonActionPerformed
        rootCAs = new HashSet<X509CertificateHolder>();
        updateCrtPathTab();
    ...
    private void clearImdsButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_clearImdsButtonActionPerformed
        imds = new HashSet<X509CertificateHolder>();
        updateCrtPathTab();
    }//GEN-LAST:event_clearImdsButtonActionPerformed

    private void displayX509CertificateHashCodes(String preMsg, List<X509CertificateHolder> x509s) throws CertificateEncodingException, CertificateParsingException {
        {
            int i;
            int n;
            ...
            for (i = 0; i < n; i++) {
                xh = x509s.get(i);
                if(xh == null){
                    pth.write("Could not decode X509Certificate\n");
                    continue;
                }
                xi = X509Inspector.newX509Inspector(xh);
                hashCode = xh.hashCode();
                String subjName = (xi.getSubjectName() == null) ? "null" : xi.getSubjectName();
                String issuerName = (xi.getIssuerName() == null) ? "null" : xi.getIssuerName();
    ...
    private void displayRootImdHashCodesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_displayRootImdHashCodesActionPerformed
        try {
            displayX509CertificateHashCodes("rootCAs:", new ArrayList<X509CertificateHolder>(rootCAs));
            displayX509CertificateHashCodes("intermediates:", new ArrayList<X509CertificateHolder>(imds));
        } catch (CertificateEncodingException | CertificateParsingException ex) {
            pth.writeException(ex);
            return;
        }
    ...
    private void displayCrtHashCodeButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_displayCrtHashCodeButtonActionPerformed
        try {
            X509CertificateHolder userCrt = readCrtPathFN();
            ArrayList<X509CertificateHolder> userCrtAsSingleList = new ArrayList<X509CertificateHolder>();
            if (userCrt != null) {
    ...
    private void fetchServerCrtsButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_fetchServerCrtsButtonActionPerformed
        ...
        List<X509CertificateHolder> serverCrts;
        try {
            serverCrts = X509ReaderWriter.getX509CertificateHolderFromSSLServer(url);
        } catch (X509ReaderException ex) {
            dbg.writeException(ex);
            return;
        }

        for (X509CertificateHolder x509obj : serverCrts) {
    ...
    private void displayX509CertificateObject(X509CertificateHolder x509obj) {
        try {
    ...
    private Set<X509CertificateHolder> rootCAs;
    private Set<X509CertificateHolder> imds;
