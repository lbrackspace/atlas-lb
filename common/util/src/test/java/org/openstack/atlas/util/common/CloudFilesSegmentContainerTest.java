...
public class CloudFilesSegmentContainerTest {
    ...
    @Test
    public void testHourMutex() throws InterruptedException {
        int hourKey = StaticDateTimeUtils.DateTimeToHourKeyInt(StaticDateTimeUtils.nowDateTime(true));
        Assert.assertTrue(CloudFilesSegmentContainer.lockUploadHour(hourKey));
        Assert.assertFalse(CloudFilesSegmentContainer.lockUploadHour(hourKey));
        Assert.assertFalse(CloudFilesSegmentContainer.lockUploadHour(hourKey));
        CloudFilesSegmentContainer.unlockUploadHour(hourKey);
        Assert.assertTrue(CloudFilesSegmentContainer.lockUploadHour(hourKey));
        CloudFilesSegmentContainer.unlockUploadHour(hourKey);
    }
