...
public class CloudFilesSegmentContainer {
    ...
    private static final Set<Integer> uploadMutex;
    private static final Set<Integer> deleteMutex;

    static {
        uploadMutex = new HashSet<Integer>();
        deleteMutex = new HashSet<Integer>();
    }
    ...
    public static boolean lockUploadHour(int hourKey) {
        boolean isAlreadyWorkingOnHour;
        synchronized (CloudFilesSegmentContainer.uploadMutex) {
            isAlreadyWorkingOnHour = uploadMutex.contains(hourKey);
            if (isAlreadyWorkingOnHour) {
                return false;
            }
            uploadMutex.add(hourKey);
            return true;
        }
    }

    public static void unlockUploadHour(int hourKey) {
        synchronized (CloudFilesSegmentContainer.uploadMutex) {
            uploadMutex.remove(hourKey);
        }
    }

    public static boolean lockDeleteHour(int hourKey) {
        boolean isAlreadyWorkingOnHour;
        synchronized (CloudFilesSegmentContainer.deleteMutex) {
            isAlreadyWorkingOnHour = deleteMutex.contains(hourKey);
            if (isAlreadyWorkingOnHour) {
                return false;
            }
            uploadMutex.add(hourKey);
    ...
    public static void unlockDeleteHour(int hourKey) {
        synchronized (CloudFilesSegmentContainer.deleteMutex) {
            deleteMutex.remove(hourKey);
        }
    }
