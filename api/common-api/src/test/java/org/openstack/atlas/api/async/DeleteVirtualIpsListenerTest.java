...
public class DeleteVirtualIpsListenerTest extends VTMTestBase {
    ...
    public void standUp() {
        ...
        deleteVirtualIpsListener.setReverseProxyLoadBalancerVTMService(reverseProxyLoadBalancerVTMService);
    ...
    public void testDeleteVirtualIps() throws Exception {
        ...
        verify(reverseProxyLoadBalancerVTMService).deleteVirtualIps(lb, vipIdsToDelete, null);
...
//        verify(usageEventHelper).processUsageEvent(eq(lb), eq(UsageEvent.DELETE_VIRTUAL_IP), Matchers.any(Calendar.class));
    ...
    public void testDeleteInvalidZeusVirtualIps() throws Exception {
        ...
        when(messageDataContainer.getIds()).thenReturn(vipIdsToDelete);
        when(loadBalancerService.getWithUserPages(LOAD_BALANCER_ID, ACCOUNT_ID)).thenReturn(lb);
        doThrow(Exception.class).when(reverseProxyLoadBalancerVTMService).deleteVirtualIps(lb, vipIdsToDelete, null);
        when(config.getString(Matchers.<ConfigurationKey>any())).thenReturn("REST");
        ...
        verify(notificationService).saveAlert(eq(ACCOUNT_ID), eq(LOAD_BALANCER_ID), isA(Exception.class), eq(AlertType.ZEUS_FAILURE.name()), eq(String.format("Error deleting virtual ips backend.. for loadbalancer '%d'.", lb.getId())));
        verify(notificationService).saveVirtualIpEvent(eq(USERNAME), eq(ACCOUNT_ID), eq(LOAD_BALANCER_ID), anyInt(), anyString(), anyString(), eq(EventType.DELETE_VIRTUAL_IP), eq(CategoryType.DELETE), eq(EventSeverity.CRITICAL));
    }
