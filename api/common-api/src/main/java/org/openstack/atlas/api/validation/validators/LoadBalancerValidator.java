        ...
        validator = build(new ValidatorBuilder<LoadBalancer>(LoadBalancer.class) {
            {
                ...
                result(validationTarget().getVirtualIps().getVirtualIps()).must().exist().forContext(POST).withMessage("Must provide at least one virtual ip for the load balancer.");
                ...
                result(validationTarget().getAccessList().getNetworkItems()).if_().exist().then().must().cannotExceedSize(100).withMessage("Must not provide more than one hundred access list items");
                result(validationTarget().getAccessList().getNetworkItems()).if_().exist().then().must().delegateTo(new NetworkItemValidator().getValidator(), POST).forContext(POST);
                result(validationTarget().getNodes().getNodes()).must().exist().forContext(POST).withMessage("Must provide at least one node for the load balancer.");
                result(validationTarget().getNodes().getNodes()).must().adhereTo(new DuplicateNodeVerifier()).forContext(POST).withMessage("Duplicate nodes detected. Please ensure that the ip address and port are unique for each node.");
                result(validationTarget().getNodes().getNodes()).must().adhereTo(new ActiveNodeVerifier()).forContext(POST).withMessage("Please ensure that at least one node has an ENABLED condition.");
                result(validationTarget().getNodes().getNodes()).if_().exist().then().must().delegateTo(new NodeValidator().getValidator(), POST).forContext(POST);
                result(validationTarget().getNodes().getNodes()).must().haveSizeOfAtLeast(1).forContext(POST).withMessage("Must have at least one node.");
                result(validationTarget().getMetadata().getMetas()).if_().exist().then().must().delegateTo(new MetaValidator().getValidator(), POST).forContext(POST);
                result(validationTarget().getMetadata().getMetas()).if_().exist().then().must().adhereTo(new DuplicateMetaVerifier()).forContext(POST).withMessage("Duplicate nodes detected. Please ensure that the ip address and port are unique for each node.");
                result(validationTarget().getMetadata().getMetas()).if_().exist().then().must().cannotExceedSize(25).withMessage("Must not provide more than twenty five metadata items per load balancer.");
                ...
                result(validationTarget().getNodes().getNodes()).if_().exist().then().must().cannotExceedSize(25).withMessage("Must not provide more than twenty five nodes per load balancer.");
                ...
                result(validationTarget().getNodes().getNodes()).must().beEmptyOrNull().forContext(PUT).withMessage("Please visit {account id}/loadbalancers/{load balancer id}/nodes to configure nodes.");
                ...
                result(validationTarget().getVirtualIps().getVirtualIps()).must().beEmptyOrNull().forContext(PUT).withMessage("Please visit {account id}/loadbalancers/{load balancer id}/virtualips/{virtual ip id} to configure a virtual ip.");
                ...
                result(validationTarget().getAccessList().getNetworkItems()).must().beEmptyOrNull().forContext(PUT).withMessage("Please visit {account id}/loadbalancers/{load balancer id}/accesslist to configure access lists.");
