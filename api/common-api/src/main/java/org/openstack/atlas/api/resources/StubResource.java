...
public class StubResource extends CommonDependencyProvider {
    ...
    public Response stubCertificateMappings() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            cm.setId(i);
            cm.setHostName(String.format("www.host_%i.org", i));
            cm.setCertificate(String.format("certificate_%i", i));
            cm.setIntermediateCertificate(String.format("imd_%i", i));
            cm.setPrivateKey(String.format("private key_%i", i));
