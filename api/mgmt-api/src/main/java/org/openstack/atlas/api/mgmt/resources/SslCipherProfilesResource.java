...
public class SslCipherProfilesResource extends ManagementDependencyProvider {
    ...
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public Response getCipherProfileByName(@PathParam("profilename") String profileName) {
        if (!isUserInRole("cp,ops,support")) {
            return ResponseFactory.accessDenied();
        }
        org.openstack.atlas.service.domain.entities.SslCipherProfile domainSslCipherProfile;
        org.openstack.atlas.docs.loadbalancers.api.management.v1.SslCipherProfile dataModelSslCipherProfile;
        try {
            domainSslCipherProfile = sslCipherProfileService.getByName(profileName);
