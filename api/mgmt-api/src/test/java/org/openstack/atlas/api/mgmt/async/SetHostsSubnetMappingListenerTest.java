...
public class SetHostsSubnetMappingListenerTest extends VTMTestBase {
    ...
    @Test
    public void testSetSubnetMappingsInCaseOfSingleHostInList() throws Exception {
        ...
        when(esbRequest.getVirtualIpType()).thenReturn(VirtualIpType.PUBLIC);
        when(config.getString(Matchers.<ConfigurationKey>any())).thenReturn("REST");
        doNothing().when(reverseProxyLoadBalancerVTMService).setSubnetMappings(ArgumentMatchers.<Host>any(), ArgumentMatchers.<Hostssubnet>any());
        mgmtSetHostsSubnetMappingListener.doOnMessage(objectMessage);
        verify(reverseProxyLoadBalancerVTMService, times(1)).setSubnetMappings(ArgumentMatchers.eq(host1), ArgumentMatchers.any());
        verify(reverseProxyLoadBalancerVTMService, times(0)).deleteSubnetMappings(ArgumentMatchers.eq(host1), ArgumentMatchers.any());
        verify(notificationService, times(0)).saveAlert(any(), any(), any());
        verify(clusterService, times(1)).addVirtualIpBlocks(any(), eq(VirtualIpType.PUBLIC), eq(mCluster.getId()));
    }
}