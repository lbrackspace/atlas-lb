...
public class SetHostsSubnetMappingListenerTest extends VTMTestBase {
    ...
    public void testSetSubnetMappings() throws Exception {
        ...
        when(esbRequest.getHosts()).thenReturn(hosts);
    ...
    public void testSetSubnetMappingsInCaseOfException() throws Exception {
        ...
        when(clusterService.getHosts(ArgumentMatchers.anyInt())).thenReturn(hosts);
        when(config.getString(Matchers.<ConfigurationKey>any())).thenReturn("REST");
        doThrow(RollBackException.class).when(reverseProxyLoadBalancerVTMService).setSubnetMappings(ArgumentMatchers.any(), ArgumentMatchers.any());

        mgmtSetHostsSubnetMappingListener.doOnMessage(objectMessage);
        verify(reverseProxyLoadBalancerVTMService, times(2)).getSubnetMappings(ArgumentMatchers.<Host>any());
    }
