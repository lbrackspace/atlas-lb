...
public class VTMResourceTranslator {
    ...
    public VirtualServer translateRedirectVirtualServerResource(LoadBalancerEndpointConfiguration config, String vsName, LoadBalancer loadBalancer) throws InsufficientRequestException {
        ...
        if ((loadBalancer.getAccessLists() != null && !loadBalancer.getAccessLists().isEmpty()) || loadBalancer.getConnectionLimit() != null) {
            basic.setProtectionClass(VTMNameBuilder.genVSName(loadBalancer));
        }
        ...
        if (loadBalancer.getUserPages() != null && loadBalancer.getUserPages().getErrorpage() != null) {
            ce.setErrorFile(VTMNameBuilder.generateErrorPageName(VTMNameBuilder.genRedirectVSName(loadBalancer)));
        } else {
    ...
    public VirtualServer translateVirtualServerResource(LoadBalancerEndpointConfiguration config,
        ...
        if (vsName.equals(VTMNameBuilder.genSslVSName(loadBalancer))) {
            ...
            if (loadBalancer.getCertificateMappings() != null ) {
                ...
                for (CertificateMapping cm : certMappings) {
                    ...
                    vcm.setCertificate(VTMNameBuilder.generateCertificateName(loadBalancer.getId(),
        ...
        basic.setPool(VTMNameBuilder.genVSName(loadBalancer));
        basic.setProtocol(VirtualServerBasic.Protocol.fromValue(VTMConversionUtils.mapProtocol(loadBalancer.getProtocol())));
        ...
        //error file settings
        if (loadBalancer.getUserPages() != null && loadBalancer.getUserPages().getErrorpage() != null) {
            ce.setErrorFile(VTMNameBuilder.generateErrorPageName(vsName));
        } else {
            ce.setErrorFile("Default");
        }
        properties.setConnectionErrors(ce);
    ...
    public Map<String, TrafficIp> translateTrafficIpGroupsResource(LoadBalancerEndpointConfiguration config,
        ...
        for (LoadBalancerJoinVip loadBalancerJoinVipToAdd : loadBalancer.getLoadBalancerJoinVipSet()) {
            nameandgroup.put(VTMNameBuilder.generateTrafficIpGroupName(loadBalancer, loadBalancerJoinVipToAdd.getVirtualIp()),
        ...
        for (LoadBalancerJoinVip6 loadBalancerJoinVip6ToAdd : loadBalancer.getLoadBalancerJoinVip6Set()) {
            try {
                ...
                nameandgroup.put(VTMNameBuilder.generateTrafficIpGroupName(loadBalancer, loadBalancerJoinVip6ToAdd.getVirtualIp()),
...
//        basic.setIp_mapping(mappings);
    ...
    public Set<String> genGroupNameSet(LoadBalancer loadBalancer) throws InsufficientRequestException {
        ...
        for (LoadBalancerJoinVip loadBalancerJoinVipToAdd : loadBalancer.getLoadBalancerJoinVipSet()) {
            groupSet.add(VTMNameBuilder.generateTrafficIpGroupName(acctId, loadBalancerJoinVipToAdd.getVirtualIp().getId().toString()));
        }

        for (LoadBalancerJoinVip6 loadBalancerJoinVip6ToAdd : loadBalancer.getLoadBalancerJoinVip6Set()) {
            groupSet.add(VTMNameBuilder.generateTrafficIpGroupName(acctId, loadBalancerJoinVip6ToAdd.getVirtualIp().getId().toString()));
        }
...
//            #NOTE: These values are  currently deprecated
    ...
    public Keypair translateKeypairResource(LoadBalancer loadBalancer, boolean careAboutCert)
        ...
        if (zeusCertFile.hasFatalErrors()) {
            String fmt = "VTMCertFile generation Failure: %s";
    ...
    public Map<String, Keypair> translateKeypairMappingsResource(LoadBalancer loadBalancer, boolean careAboutCert)
        ...
        for (CertificateMapping cm : loadBalancer.getCertificateMappings()) {
            ...
            String certificateName = VTMNameBuilder.generateCertificateName(lbId, accountId, cm.getId());
            // Decrypt key
            String privKey = null;
            try {
                // decrypt database key so we can revalidate with any new data
