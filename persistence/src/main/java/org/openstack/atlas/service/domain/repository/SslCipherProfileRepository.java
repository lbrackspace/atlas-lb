...
public class SslCipherProfileRepository {
    ...
    public SslCipherProfile getByName(String name) throws EntityNotFoundException {
        List<SslCipherProfile> sslCipherProfiles = entityManager.createQuery("SELECT s FROM SslCipherProfile s where s.name = :name").setParameter("name", name).getResultList();
    ...
    public List<SslCipherProfile> getMatchingByName(String name) throws EntityNotFoundException {
        List<SslCipherProfile> sslCipherProfiles = entityManager.createQuery("SELECT s FROM SslCipherProfile s where s.name like :name").setParameter("name", '%'+name+'%').getResultList();
