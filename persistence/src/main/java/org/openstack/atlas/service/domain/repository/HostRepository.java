...
public class HostRepository {
    ...
    public Host getDefaultActiveHost(Integer clusterId, Integer hostId) throws EntityNotFoundException {
        ...
        String sql = "SELECT h from Host h where h.cluster.id = :clusterId AND h.hostStatus= :hostStatus AND h.id <> :hostId "
...
//                + HostStatus.ACTIVE_TARGET + "')";
        ...
        Query qry = entityManager.createQuery(sql).setParameter("hostStatus", HostStatus.ACTIVE_TARGET).setParameter("clusterId", clusterId).setParameter("hostId", hostId);
...
//    public Host getNotCurrentActiveHost(Integer clusterId, Integer hostId) throws EntityNotFoundException {
//        //get a host based on the following algorithm
//        //status = ACTIVE_TARGET, fewest concurrent connections and fewest number of assigned loadbalanders.
//        String sql = "SELECT h from Host h where h.cluster.id = :clusterId AND h.hostStatus= :hostStatus AND h.id <> :hostId "
//                + "AND h.maxConcurrentConnections = (select min(i.maxConcurrentConnections) "
