...
public class ClusterRepository  {
    ...
    public Cluster getActiveCluster() {
        List<Cluster> cls = entityManager.createQuery("SELECT cl FROM Cluster cl where cl.cluster.status = :status").setParameter("status", ClusterStatus.ACTIVE).getResultList();
        if (cls != null && cls.size() > 0) {
            return cls.get(0);
        } else {
            return null;
