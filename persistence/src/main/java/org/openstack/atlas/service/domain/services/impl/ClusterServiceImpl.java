...
public class ClusterServiceImpl extends BaseService implements ClusterService {
    ...
    public void addVirtualIpBlocks(IPv4Ranges ranges, VirtualIpType vipType, Integer clusterId) throws EntityNotFoundException {
        LOG.debug("Entering " + getClass() + ": addVirtualIpBlocks");
        ...
        for (IPv4Range range : ranges.getRanges()) {
            for (ip = range.getLo() + 1; ip < range.getHi(); ip++) {
                VirtualIp vip = new VirtualIp();
                vip.setIpAddress(IPv4ToolSet.long2ip(ip));
                vip.setVipType(vipType);
                vip.setCluster(cluster);
                vip.setAllocated(false);
                if (verifyDuplicateVip(vip, dbVips)) {
        ...
        LOG.info(String.format("AddVirtualIpBlocks operation completed, %s vips were added", vips.size()));
}
    ...
    private boolean verifyDuplicateVip(VirtualIp vip, List<VirtualIp> dbVips) {
        for (VirtualIp nvip : dbVips) {
            if (vip.getIpAddress().equals(nvip.getIpAddress())) {
                return true;
            }
