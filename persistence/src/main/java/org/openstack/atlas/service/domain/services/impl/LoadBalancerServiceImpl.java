    ...
    private void processSpecifiedOrDefaultHost(LoadBalancer lb) throws EntityNotFoundException, BadRequestException, ClusterStatusException, NoAvailableClusterException {
        ...
        if (!lb.getIsSticky()) {
            if (hostId != null) {
                ...
                lb.setHost(hostService.getNotCurrentActiveHostAndActiveCluster(lb.getAccountId(), loadBalancerHasPublicVip(lb), loadBalancerRepository.getById(lb.getId()).getHost().getId()));
            }
        }
    }
