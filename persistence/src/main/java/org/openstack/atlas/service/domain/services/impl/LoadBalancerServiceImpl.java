...
//        notificationService.saveLoadBalancerEvent(loadBalancer.getUserName(), dbLoadBalancer.getAccountId(), dbLoadBalancer.getId(), atomTitle, atomSummary, PENDING_UPDATE_LOADBALANCER, UPDATE, INFO);
    ...
    private void verifyProtocolLogging(LoadBalancer loadBalancer, LoadBalancer dbLoadBalancer) throws UnprocessableEntityException {
        String logErr = "Protocol must be a non-UDP protocol for connection logging to be enabled.";
        String proErr = "Cannot change protocol from non-UDP to UDP while logging is enabled.";
        ...
        if (changingProtocol && changingLogging) {
            if(loadBalancer.isUdpProtocol() && loadBalancer.isConnectionLogging()) {
                LOG.error(logErr);
            ...
            if (loadBalancer.isConnectionLogging()) {
                if (dbLoadBalancer.isUdpProtocol()) {
                    LOG.error(logErr);
                    throw new UnprocessableEntityException(logErr);
                }
                LOG.debug("ConnectionLogging is being enabled on the loadbalancer.");
