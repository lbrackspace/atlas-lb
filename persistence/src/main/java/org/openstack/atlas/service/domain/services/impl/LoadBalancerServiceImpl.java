...
public class LoadBalancerServiceImpl extends BaseService implements LoadBalancerService {
    ...
    public LoadBalancer create(LoadBalancer lb) throws Exception {
        ...
        // Check for creation of non-HTTP load balancer with content caching enabled
        if (lb.isContentCaching() != null && lb.isContentCaching() && lb.getProtocol() != LoadBalancerProtocol.HTTP) {
            String message = "Protocol must be HTTP for content caching to be enabled.";
            LOG.warn(message);
            throw new BadRequestException(message);
        }
