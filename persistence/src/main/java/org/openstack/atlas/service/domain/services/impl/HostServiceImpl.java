...
//       TODO: Make Zeus call here       "In Activating Host in ZEUS.. TODO: No Zeus call yet :(");
    ...
    public RegionalSourceAddresses getRegionalSourceAddresses(ClusterType cType) {
        ...
        for (Host h : hosts) {
            rsa.getIpv4Publicnets().add(h.getIpv4Public());
            rsa.getIpv4Servicenets().add(h.getIpv4Servicenet());
            rsa.getIpv6Publicnets().add(h.getIpv6Public());
            rsa.getIpv6Servicenets().add(h.getIpv6Servicenet());
        }
        return rsa;
    }
