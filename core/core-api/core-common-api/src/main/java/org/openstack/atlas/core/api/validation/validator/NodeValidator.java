...
import org.openstack.atlas.core.api.validation.verifier.*;
...
import static org.openstack.atlas.core.api.validation.context.HttpRequestType.POST;
        ...
        validator = build(new ValidatorBuilder<Node>(Node.class) {
            {
                ...
                result(validationTarget().getCondition()).if_().exist().then().must().adhereTo(new MustBeInArray(NodeCondition.toArray())).withMessage("Node condition is invalid. Please specify a valid condition.");
